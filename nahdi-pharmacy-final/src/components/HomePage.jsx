  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        console.log('📦 جار جلب المنتجات المميزة...');
        const productsResponse = await productService.getFeaturedProducts();
        console.log('✅ استجابة المنتجات من السيرفر:', productsResponse);
        ...
        if (Array.isArray(productsResponse)) {
          ...
          // تحقق من وجود حقل data وانه مصفوفة
          productsData = Array.isArray(productsResponse.data) 
            ? productsResponse.data 
            : [];
        }
        
        console.log('📊 بيانات المنتجات المستخرجة:', productsData);
