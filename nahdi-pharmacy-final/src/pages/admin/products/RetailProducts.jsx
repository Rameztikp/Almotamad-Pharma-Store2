...
const RetailProducts = () => {
  ...
  const fetchProducts = async () => {
    try {
      ...
      // The response is already the data we need, no need for response.data
      ...
      const retailProducts = productsList.filter(
        product => product.type === 'retail' || product.type === undefined
      );
  ...
  const handleFormSubmit = async (productData) => {
    try {
      // Add product type to the data
      const productToSave = {
        ...productData,
        type: 'retail' // Set the product type
      };
      
      // Call the API to create the product
      const response = await productService.createProduct(productToSave);
  ...
  return (
    <div className="p-4">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold text-gray-800">منتجات القطاعي</h2>
        <button
          onClick={() => {
            setCurrentProduct(null);
            setIsFormOpen(true);
